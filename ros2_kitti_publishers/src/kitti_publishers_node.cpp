...
KittiPublishersNode::KittiPublishersNode(const ros::NodeHandle &nh)
: nh_(nh),file_index_(0)
{ 
  publisher_point_cloud_ = nh_.advertise<sensor_msgs::PointCloud2>("kitti/point_cloud", 10);
  publisher_image_gray_left_ = nh_.advertise<sensor_msgs::Image>("kitti/image/gray/left", 10);
  publisher_image_gray_right_ = nh_.advertise<sensor_msgs::Image>("kitti/image/gray/right", 10);
  publisher_image_color_left_ = nh_.advertise<sensor_msgs::Image>("kitti/image/color/left", 10);
  publisher_image_color_right_ = nh_.advertise<sensor_msgs::Image>("kitti/image/color/right", 10);
  publisher_imu_ = nh_.advertise<sensor_msgs::Imu>("kitti/imu", 10);
  publisher_nav_sat_fix_= nh_.advertise<sensor_msgs::NavSatFix>("kitti/nav_sat_fix", 10);
  publisher_marker_array_ = nh_.advertise<visualization_msgs::MarkerArray>("kitti/marker_array", 10);

  init_file_path();

  create_publishers_data_file_names();

  timer_ = nh_.createTimer(ros::Duration(0.1), &KittiPublishersNode::on_timer_callback,this);
}

void KittiPublishersNode::on_timer_callback(const ros::TimerEvent& event)
{
    ...
    sensor_msgs::Image image_message_gray_left;
    ...
    sensor_msgs::Image image_message_gray_right;
    ...
    sensor_msgs::Image image_message_color_left;
    ...
    sensor_msgs::Image image_message_color_right;
    ...
    //ROS_INFO("OxTs size: '%i'", oxts_parsed_array.size());
    ...
    publisher_image_gray_left_.publish(image_message_gray_left);
    publisher_image_gray_right_.publish(image_message_gray_right);
    publisher_image_color_left_.publish(image_message_color_left);
    publisher_image_color_right_.publish(image_message_color_right);

    publisher_imu_.publish(imu_msg);
    publisher_nav_sat_fix_.publish(nav_sat_fix_msg);
    publisher_marker_array_.publish(marker_array_msg);

    file_index_++;
}

void KittiPublishersNode::convert_pcl_to_pointcloud2(sensor_msgs::PointCloud2 & msg ){
    ...
    if(!input.good()){
      ROS_INFO("Could not read Velodyne's point cloud. Check your file path!");
      file_index_=0;
      // return;
      // exit(EXIT_FAILURE);
    }
...
{
    path_point_cloud_ = "/home/jylau810/point_cloud_processing/src/data/2011_09_26/2011_09_26_drive_0001_sync/velodyne_points/data/";
    path_image_gray_left_ = "/home/jylau810/point_cloud_processing/src/data/2011_09_26/2011_09_26_drive_0001_sync/image_00/data/";
    path_image_gray_right_ = "/home/jylau810/point_cloud_processing/src/data/2011_09_26/2011_09_26_drive_0001_sync/image_01/data/";
    path_image_color_left_ = "/home/jylau810/point_cloud_processing/src/data/2011_09_26/2011_09_26_drive_0001_sync/image_02/data/";
    path_image_color_right_ = "/home/jylau810/point_cloud_processing/src/data/2011_09_26/2011_09_26_drive_0001_sync/image_03/data/";
    path_oxts_ = "/home/jylau810/point_cloud_processing/src/data/2011_09_26/2011_09_26_drive_0001_sync/oxts/data/";
}

std::string KittiPublishersNode::get_path(KittiPublishersNode::PublisherType publisher_type)
{
  //ROS_INFO("get_path: '%i'", publisher_type);
  std::string path;
...
void KittiPublishersNode::create_publishers_data_file_names()
{
  for ( int type_index = 0; type_index <6; type_index++ )
  {
   ...
   try
   {
    ...
    {
        ROS_ERROR("File path not found.");
    }
  }
}


void KittiPublishersNode::prepare_navsatfix_msg(std::vector<std::string> &oxts_tokenized_array, sensor_msgs::NavSatFix &msg)
{
  msg.header.frame_id = "map";
  msg.header.stamp = ros::Time::now();
...
void KittiPublishersNode::prepare_marker_array_msg(std::vector<std::string> &oxts_tokenized_array, visualization_msgs::MarkerArray &msg)
{
...
void KittiPublishersNode::prepare_imu_msg(std::vector<std::string> &oxts_tokenized_array, sensor_msgs::Imu &msg){
  msg.header.frame_id = "map";
  msg.header.stamp = ros::Time::now();
...
void KittiPublishersNode::convert_image_to_msg(sensor_msgs::Image & msg, const std::string path  )
{
  Mat frame;
  frame = imread(path);
  if (frame.empty())                      // Check for invalid input
  {
    ROS_ERROR("Image does not exist. Check your files path!");
    ros::shutdown();
  }
...
std::vector<std::string> KittiPublishersNode::parse_file_data_into_string_array(std::string file_name, std::string delimiter)
{
    ...
    if(!f.good()){
      ROS_INFO("Could not read OXTS data. Check your file path!");
      exit(EXIT_FAILURE);
    }
...
int main(int argc, char ** argv)
{
  ...
  ros::NodeHandle nh;
  KittiPublishersNode controller(nh);
